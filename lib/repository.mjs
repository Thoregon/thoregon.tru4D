...
*/
    ...
    async withIdentity() {
        ...
        controller.on('auth',   async (event) => await this.signon(event.identity));
        controller.on('leave',  async (event) => await this.signoff(event.identity));
    }

    async signon(idenity) {
        this.identity = idenity;
        await forEach(this.contexts.values(), async (context) => await context.signon(idenity));
    }

    async signoff(idenity) {
        this.identity = idenity;
        await forEach(this.contexts.values(), async (context) => await context.signoff(idenity));
    }
