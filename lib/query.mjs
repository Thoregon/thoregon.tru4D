 ...
 *
 *  todo [REFACTOR]
 *   - add 'after' in the modification -> allow receiver to sort
 *   - introduce query state machine
 *   - add queue for deferred processing when query is not yet ready (initializing, working, ...)
 *   - fluent interface for filter (find) and sort
 *      - query = Query.at(x).filter(fn)
 *      - await query.next() (implement also async iterator)
 *      - 2 API's
 *          - PULL: one time use with pull requests (like PreparedStatement)
 *          - PUSH: connected query with push results (event triggered)
 *   - indexes
 *      - at -> with
 *      - @later: manage multiple indexes
 *   - direct on matter -> meta
 *   - active parms
 *      - can directly connected with UI elements
 *      - change of param tiggers query
 *
 *
...
import AsyncResource     from "/evolux.util/lib/asyncresource.mjs";
...
export default class Query extends AsyncResource() {
    ...
    dispatch(modification) {
        ...
        if (this._items.has(id)) {
            ...
            let event = { type: 'add', id, item: oitem };   // todo: 'after' ... for sorting, if missing it's the very first item
            if (this._listener) this._listener(event);
        }
    }
