...
import { ErrCommandAlreadyComitted, ErrNoId, ErrCommandHasNoSchema }       from "../errors.mjs";
...
export default class Command {
     ...
     * @return {Command}
     */
    static useKey(object) {
        let cmd = new this(object);
        let schema = this.schema;
        if (!schema) throw ErrCommandHasNoSchema(className(cmd));
        let key = schema.keyFrom(object);
        if (!key) throw ErrNoId(className(cmd));
        cmd.key = key;
        return cmd;
    }
