...
export default class Command {
     ...
     */
    constructor(ctxid, responsibility, cmdid) {
        this.ctxid          = ctxid;
        if (responsibility) this.responsibility = responsibility;
        if (cmdid)          this.commandid = cmdid;
    }
    ...
    async commit() {
        ...
        if (this.commandid) {
            ...
            cmd.command  = this.commandid;
        }
        if (this.key)       cmd.key = this.key;
        if (this.parent)    cmd.parent = this.parent;
        if (this.auto)      cmd.auto = true;

        // let bounded context instance work, return the Promise to handle success/error
        return this.tru4d.context(this.ctxid).commitCommand(this, cmd);
    }

    results() {
        if (!this.cmd && !this.cmd.cmdid) return undefined;
        return myuniverse().matter.root[this.cmd.cmdid].results;
    }
     ...
     */
    async prepare() {
        if (this.doValidate() && this.hasSchema()) {
    ...
    doValidate() {
        return false; // this.constructor.validate
    }
