 ...
 *          - private
 *            - {identity}      -> can be accessed by identity, encrypted
 *              - {collectionname}
 *          - shared
...
import { ErrInvalidBoundedContext }         from "../errors.mjs";
...
export default class BoundedContext extends Reporter(EventEmitter) {
    ...
    async getCollection(name) {
        ...
        let pcoll       = await collnode.val;           // if there was no persistent information create it
        if (!pcoll) {
    ...
    getCollectionNode(name) {
        ...
        switch (scope) {
            ...
            case 'shared':
                path = 'shared';
                break;
            otherwise: // should be -> case 'public':
                break;
        }
    ...
    async resolve(commanddata) {
        ...
        let cmd = { command: commanddata.command, cmdid: commanddata.cmdid };
    ...
    actionsFor(command) {
        let cmdactions = this.cmdactions[command];
