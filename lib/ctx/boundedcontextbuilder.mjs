...
export default class BoundedContextBuilder extends Reporter(EventEmitter) {
     ...
     * @return {BoundedContextBuilder}
     */
    addAction(commandselector, action) {
        const item = this._last();
        ...
        let reg = item.actions[commandselector];
        if (!reg) {
            reg = [];
            this.actions[commandselector] = reg;
        }
        reg.push(action);
        return this;
    }

    withAction(actionid, action, commandselector) {
        if (commandselector) action.selector = commandselector;
        let item = { actions: [{ cmdid: actionid, action }] };
    ...
    async build() {
        ...
        await forEach(this._steps, async item => {
            await this._buildSchemaItems(item, bc);
        });
    ...
    async _buildSchemaItems(item, bc) {
        ...
        if (item.actions) {
            item.actions.forEach(action => {
                bc.useAction(action.id, action.action);
            })
        }
    }
