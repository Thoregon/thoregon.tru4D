...
export default class BoundedContextBuilder extends Reporter(EventEmitter) {
     ...
     */
    addCommand(commandid, responsibility) {
        const item = this._last();
        let command;
        if (is.string(commandid)) {
            command = this._asCommandClass(commandid);
     ...
     * @param commandid     ... name (id) of the command (with namespace)
     * @param action        ... the action for the command
     * @return {BoundedContextBuilder}
     */
    addAction(commandid, action) {
        const item = this._last();
        let cmd = item.commands[commandid];
        if (!cmd) {
            this.logger.error(`${this._name}, command '${commandid}' for action not found`);
            return this;
        }
        let reg = item.actions[commandid];
        if (!reg) {
            reg = [];
            this.actions[commandid] = reg;
        }
        reg.push(action);
        return this;
    }
    ...
    async build() {
        ...
        await forEach(this._steps, async item => {
            if (item.schema) await this._buildSchemaItems(item, bc);
        });
     ...
     */
    async _buildSchemaItems(item, bc) {
        const ctxid = bc.name;
        ...
        item.commands.forEach(command => {
            let id = command.id || `${ctxid}.${className(command)}`;
            bc.useCommand(id, command);
        });

        item.actions.forEach(action => {
            let id = action.id || `${ctxid}.${className(action)}`;
            bc.useCommand(id, action);
        });
    }
