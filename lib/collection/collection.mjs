...
export default class Collection extends CollectionUtil(Reporter(EventEmitter)) {
    ...
    constructor({
        ...
        this.nodes      = [];               // contains matter nodes from items
    ...
    async add(object) {
        ...
        return result;  // return the node from matter
    }
...
*/
        ...
        await forEach(dbkeys, async (key) => {
            // maintain nodes
            nodes.push(content[key]);
            // maintain items
            let item = await content[key].full;
            items.push(item);
            // maintain idxmap
        ...
        // maintain pk
        if (this.hasPK()) {
            let pks     = await this.node.pk.val || {};
            let schema  = this.schema;
            let itemmap = {};
            for (let i = 0; i < items.length; i++) {
                let pk = schema.keyFrom(items[i]);
                itemmap[pk] = nodes[i].node;
            }
            ...
            removed.forEach(key => this.node.pk[key].put(null));
            added.forEach(key => this.node.pk[key] = itemmap[key]);
        }
