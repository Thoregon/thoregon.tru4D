...
class LogicalBuilder extends Builder {
    ...
    async build() {
        ...
        schema.name = this._schemaName;
        if (this._extend) schema.extend = this._extend;
        if (this._impl) schema.impl = this._impl;
...
class SpecifierBuilder extends ItemBuilder {
    ...
    spec() {
        return { name: this._name, default: this._default, type: this._type, notnull: !!this._notnull };
    }
}
