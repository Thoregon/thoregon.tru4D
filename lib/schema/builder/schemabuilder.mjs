 ...
 *
 * todo [OPEN]
 *  - find clear names for schema types (component, entity, view, ...)
 *
...
export default class SchemaBuilder {
    ...
    static actions(name) {
        return new ActionsBuilder().schemaName(name);
    }
...
class ChildBuilder extends Builder {
    ...
    build(category, schema) {
        let cat = `@${category}`;
        let elems = this[cat];
        Object.entries(elems).forEach(([name, builder]) => {
            builder.build(category, name, schema);
        });
    }
}
...
class Meta extends ChildBuilder {
    ...
    build(category, schema) {
        ...
        schema.meta.version = this._version;
    }
}
...
class Params extends Specifiers {
    ...
    build(name, callable) {
        return super.build('parameters', callable);
    }
}
...
class CallableBuilder extends ItemBuilder {
    ...
    build(category, name, schema) {
        ...
        if (callable && this['@params']) this['@params'].build(name, callable);
    }
...
class ActionsBuilder extends Builder {

    section(name) {
        return ActionBuilder.under(this).name(name);
    }
...
class ActionChildBuilder {

    section() {
        return this._parent.section();
    }
}

class ActionBuilder extends ActionChildBuilder {
